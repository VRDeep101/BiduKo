...
export function ServicesSection() {
  ...
  useEffect(() => {
    if (!serviceWordRef.current) return;
    gsap.fromTo(serviceWordRef.current,
      { y: "0.35em", opacity: 0, filter: "blur(8px)" },
      { y: 0, opacity: 1, filter: "blur(0px)", duration: .5, ease: "power3.out" }
    );
  }, [activeWord]);
    ...
    <section ref={ref} id="services" className="services section-shell">
      ...
      <div className="section-heading-row services-intro">
        <div>
          ...
          <h2 className="services-headline"><span>Everything your</span><span ref={serviceWordRef} className="service-word-swap" aria-live="polite">{serviceWords[activeWord]}</span><span>requires</span></h2>
        </div>
...
export function ProjectsSection() {
  ...
  return (
    <section ref={ref} id="projects" className="projects section-shell">
      <button type="button" className="selected-work-transition" aria-label="Selected work"><MousePointer2 size={28} /><span>SELECTED WORK</span></button>
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        cards.forEach((card, index) => {
          ...
          if (index === cards.length - 1) {
            tl.to({}, { duration: .08 })
              .to(".process-intro, .process-content > .section-label", { opacity: 0, filter: "blur(14px)", y: -20, duration: .14, ease: "power2.in" });
          }
        });
      });
      return () => mm.revert();
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        gsap.set(".launch-ship-title", { opacity: 0, x: -70, filter: "blur(14px)" });
        ...
        tl.to(".launch-rocket-wrap", { y: 0, opacity: 1, scale: 1, duration: .16, ease: "power3.out" })
          .to(".launch-ship-title", { opacity: 1, x: 0, filter: "blur(0px)", duration: .13, ease: "power3.out" }, "+=.04")
          ...
          .to(".launch-ship-title, .launch-word", { opacity: 0, filter: "blur(10px)", duration: .11, ease: "power2.in" }, "<")
  ...
  return (
    <section ref={ref} className="launch-scene">
      <div className="launch-stage">
        ...
        <div className="launch-copy" aria-hidden="true">
